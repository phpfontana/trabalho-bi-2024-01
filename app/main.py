...
# Load the data
df = spark.read.csv(path="data/2nd_test/2nd_test", sep="\t", inferSchema=True)
...
df_grouped = df.groupBy("timestamp").agg(
    mean(abs(df["Bearing 1"])).alias("mean_abs_Bearing 1"),
    mean(abs(df["Bearing 2"])).alias("mean_abs_Bearing 2"),
    mean(abs(df["Bearing 3"])).alias("mean_abs_Bearing 3"),
    mean(abs(df["Bearing 4"])).alias("mean_abs_Bearing 4")
).orderBy("timestamp")
